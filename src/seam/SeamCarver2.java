...
/*************************************************************************
 * Second week coursera course Java Algorithms part 2 Seam Carving
 *
 * @Date 26.01.14
 ...
 *         http://algs4.cs.princeton.edu/windows/ Compilation: javac-algs4
 *         SeamCarver.java Execution: java-algs4 SeamCarver
 *         <name_of_image_file.png> with uncommented main method Execution only
 *         with installed algs4.jar and stdlib.jar
 *
 *         Seam Carving algorithm for image resizing
 *
 *         TODO Optimization: can not recreate picture each time we invoke
 *         removeSeam(). We can recreate image only when we invoke picture() But
 *         we should store colors of the image then. More details in checklist
...
public class SeamCarver2 {
	...
	public double energy(int x, int y) {
		if (x < 0 || y < 0 || x > width() - 1 || y > height() - 1) {
			throw new java.lang.IndexOutOfBoundsException("Bad argument value!");
		}
		if (x == 0 || y == 0 || x == width() - 1 || y == height() - 1) {
	 ...
	 *
	 * @return sequence of row indices (one per each col) for the seam
	 */
	...
	private void relaxEdge(int fromPixel, int toPixel, double[] distTo, int[] edgeTo) {
		...
		if (distTo[fromPixel] + picEnergy[colFromNumber(toPixel)][rowFromNumber(toPixel)] < distTo[toPixel]) {
			distTo[toPixel] = distTo[fromPixel] + picEnergy[colFromNumber(toPixel)][rowFromNumber(toPixel)];
	 ...
	 *
	 * @return sequence of col indices (one per each row) for the seam
	 */
	 ...
	 *
	 * @param a
	 *            sequence of row indices (one per each col) for the seam to
	 *            remove
	 */
	// remove horizontal seam from current picture
	public void removeHorizontalSeam(int[] a) {
		...
		for (int col = 0; col < width(); col++) {
			...
			// System.arraycopy(picEnergy[col], 0, updatedPicEnergy[col], 0,
			// a[col]);
	 ...
	 *
	 * @param a
	 *            sequence of col indices (one per each row) for the seam to
	 *            remove
	 */
	// remove vertical seam from current picture
